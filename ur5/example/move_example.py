...
class VREP_UR5:
    def __init__(self):
        ...
        self.tip_position_sub = rospy.Subscriber('arm_position', Float32MultiArray, self.callback_position)
        ...
        self.position.data = np.array([0.4, 0, 1.425])
        # self.X = np.array([0.4, -0.03, 0.5])

        self.X = np.array([0.49, -0.03, 1.5])
        self.Y = np.array([-1])
        # self.force = 0
    ...
    def callback_force(self, msg):
        # self.force = math.sqrt(msg.data[0]**2 + msg.data[1]**2 + msg.data[2]**2)
        self.force = msg.data
        # print "force_value:", self.force

    def callback_position(self, msg):
        self.tip_position = msg.data
    ...
    def main(self):
        ...
        while True:
            ...
            if self.force == 0:
                ...
                self.X = np.append(self.X, self.tip_position)
        ...
        self.X = np.vstack((self.X, self.tip_position))
        ...
        for i in range(200):
            ...
            rospy.sleep(0.1)
        self.stop_sim()
        np.save("X_200", self.X)
        np.save("Y_200", self.Y)
