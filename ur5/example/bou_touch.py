...
class VREP_UR5:
    def __init__(self):
        ...
        self.force_sub = rospy.Subscriber('Force', Float32MultiArray, self.callback_force)
        self.init_rate = rospy.Rate(20)

        self.position = Float32MultiArray()
        self.orientation = Float32MultiArray()

        self.position.data = np.array([0.6, 0, 1.3])

        self.X = np.array([0.5, -0.1, 1.4])
        self.Y = np.array([-1])
        # self.force = 0
    ...
    def create_orientation(self, normal):
        ...
        orientation = np.array([alpha*np.pi/180, beta*np.pi/180, ganma*np.pi/180])
    ...
    def main(self):
        ...
        self.X = np.vstack((self.X, pred_position))
        ...
        for i in range(50):
            ...
            if self.force > 100:
                ...
                self.X = np.vstack((self.X, self.position.data))
