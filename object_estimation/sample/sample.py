...
def plot_environment(position_list, X):
    ...
    ax.scatter3D(X[:, 0], X[:, 1], X[:, 2], s=100, alpha=0.1)
...
if __name__=="__main__":
    ...
    # Task 1
    X2 = np.load("../data/ellipse/ellipse_po_21.npy")
    Y2 = np.zeros(len(X2))[:, None]
    T2 = 2

    # Task 2
    X1 = np.array([[0.02, 0.04, 0.5],[0.01, -0.02, 0.5]])
    Y1 = np.array([[1], [1]])
    T1 = 1
    radius = 0.2
    ...
    # plot_sphere(radius)
    # plot_environment(position_list, X2)
    # plt.pause(0.001)
    # plt.clf()
    # plt.show()
    ...
    X1         = np.append(X1, current_po, axis=0)
    ...
    for i in range(201):
            ...
            normal, direrction = gp_model.predict_direction(current_po, T1)
            ...
            mm2, ss2 = gp_model.predict(XX, T1)
            ...
            # if i % 50 == 0:
                # mm2, ss2 = gp_model.predict(XX, T1)
                # mm2_x, mm2_y, mm2_z, ss2, error, var_ave = mean_0_position(X, Y, Z, mm2, ss2, radius)
                # np.save("../data/mtgpis_200_per5/mtgpis_task1_{}".format(i), [mm2_x, mm2_y, mm2_z, ss2, error, var_ave])
    ...
    np.save("../data/mtgpis_error_200_21", error_list)
    np.save("../data/mtgpis_var_ave_200_21", var_ave_list)
