...
def mean_0_position(x,y,z, mm, ss, r):
    ...
    for i in range(len(x)):
        for j in range(len(x)):
            ...
            tmp = np.sqrt(np.linalg.norm(r**2 - x[i][j][mm0[i][j]]**2 - y[i][j][mm0[i][j]]**2))
            z_t.append(tmp)
...
def judge_ellipse(position, r):
    # r = 0.28
    x,y,z = position
    judge = (x**2/((2*r)**2) + y**2/((2*r)**2) + z**2) - r**2
    if judge < 0: # in
        return -1
    elif judge > 0 and round(judge, 4) == 0: # on
        return 0
...
if __name__=="__main__":
    ...
    X1 = np.load("../data/ellipse_po_30.npy")

    Y1 = np.zeros(len(X1))[:, None]
    T1 = 0

    # Task 2
    X2 = np.array([[0.01, 0.01,0.4], [-0.002, -0.003, 0.4]])
    Y2 = np.array([[1], [1]])
    T2 = 1
    radius = 0.2
    ...
    kernel = InverseMultiquadricKernel([0.5])
    ...
    current_po    = np.array([0.01, 0.01, 0.35])
    position_list = np.array([current_po])

    while True:
        if judge_sphere([current_po[0], current_po[1], current_po[2]-0.002], radius) == -1:
            break
    ...
    for i in range(50):
            ...
            gp_model = MultiTaskGaussianProcessImplicitSurfaces([X1,X2], [Y1,Y2], [T1,T2], kernel)
            ...
            judge = judge_sphere(current_po[0], radius)
            print "judge:", judge
            ...
            if judge == 0:
                ...
                while True:
                    if judge_sphere((current_po - 0.0005 * normal.T)[0], radius) == -1:
                        X2 = np.append(X2, current_po, axis=0)
                        Y2 = np.append(Y2, [0])[:,None]
                        break
                ...
                while True:
                    if judge_sphere(current_po[0], radius) == 1:
                        X2 = np.append(X2, current_po, axis=0)
                        Y2 = np.append(Y2, [0])[:,None]
                        break
                    current_po += 0.0005 * normal.T
    ...
    np.save("../data/mtgpis_error_50_small", error_list)
    np.save("../data/mtgpis_var_ave_50_small", var_ave_list)
    ########################################## Plot ###################################################################
    ax = fig.add_subplot(111, projection='3d')
    plot_environment(position_list, X1)
    mtgpis_plot_3d([mm2_x, mm2_y, mm2_z], ss2)
