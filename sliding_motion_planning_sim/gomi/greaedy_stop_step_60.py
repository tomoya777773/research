...
class Path_Planning:
    ...
    def direction_func(self, current_position, orbit_position):
        ...
        diff_object = 0.3 * diff_var + 0.7 * diff_orbit + diff_penalty
...
class VREP_UR5:
    ...
    def __init__(self):
        ...
        """データを読み込み"""
        # self.X = np.load("../data/spheroid_list100.npy")
        self.X = np.load("../data/sin_surf_list.npy")
        # print self.X
        self.Y = np.zeros((self.X.shape[0], 1))

        self.orbit_position = np.load("../data/circle_r4.npy")
        self.position.data = self.orbit_position[0]
        # print self.position.data
        # print self.orbit_position
        # self.force = 0
        self.a = 0.004186446033107336
        # self.a = 0.001
    ...
    def main(self):
        ...
        for i in tqdm(range(self.orbit_position.shape[0]-1)):
            ...
            while const < 100:
                ...
                w = 20
                x = np.linspace(current_position[0] - 0.001, current_position[0] + 0.001, w)
                y = np.linspace(current_position[1] - 0.001, current_position[1] + 0.001, w)
                z = np.linspace(current_position[2] - 0.002, current_position[2] + 0.002, w)
                sample = np.array([x,y,z])

                """current_positionの近くのデータ点を収集"""
                X_ = self.X[np.where((self.X[:, 0] > current_position[0]-0.002) & (self.X[:, 0] < current_position[0]+0.002)\
                    & (self.X[:, 1] > current_position[1]-0.002) & (self.X[:, 1] < current_position[1]+0.002))]
                Y_ = self.Y[np.where((self.X[:, 0] > current_position[0]-0.002) & (self.X[:, 0] < current_position[0]+0.002)\
                    & (self.X[:, 1] > current_position[1]-0.002) & (self.X[:, 1] < current_position[1]+0.002))]
                ...
                if la == 1: # When convex
                    ...
                    n = 1
                    while n < 10:
                        new_position = normal_position + n * self.a / 10 * pred_direction
