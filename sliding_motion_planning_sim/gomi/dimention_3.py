...
class Path_Planning:
    ...
    def decision_func(self, x, y, z):
        ...
        param = [0, 0, 0, 0, 0]
        optimized_param =  optimize.leastsq(regression_func, param, args=(x, y, z))[0]
        ...
        hesse00 = diff(diff(z, x), x)
        hesse01 = diff(diff(z, y), x)
        hesse10 = hesse01.copy()
        hesse11 = diff(diff(z, y), y)

        Hesse_matrix = np.array([[hesse00,hesse01], [hesse10,hesse11]])
        la, v = np.linalg.eig(Hesse_matrix)

        cc = 0
        if (la[0] > 0 and la[1] > 0) or (la[0] < 0 and la[1] < 0):
            cc = 1
...
class VREP_UR5:
    ...
    def main(self):
        ...
        for i in tqdm(range(500)):
            ...
            x = np.linspace(current_position[0] - 1, current_position[0] + 1, w)
            y = np.linspace(current_position[1] - 1, current_position[1] + 1, w)
            z = np.linspace(current_position[2] - 1, current_position[2] + 1, w)


            X_ = self.X[np.where((self.X[:, 0] > current_position[0]-1) & (self.X[:, 0] < current_position[0]+1)\
                 & (self.X[:, 1] > current_position[1]-1) & (self.X[:, 1] < current_position[1]+1))]
            Y_ = self.Y[np.where((self.X[:, 0] > current_position[0]-1) & (self.X[:, 0] < current_position[0]+1)\
                 & (self.X[:, 1] > current_position[1]-1) & (self.X[:, 1] < current_position[1]+1))]
