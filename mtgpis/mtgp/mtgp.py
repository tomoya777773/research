...
class MultiTaskGaussianProcess:
    def __init__(self, X, Y, T, kernel, sigma=0.3):
    ...
    def predict(self, x, t):
        ...
        mean = k.T.mm(self.invK).mm(self.Y)
    ...
    def log_likehood(self):
        ...
        return  torch.logdet(K) + self.Y.T.mm(invKY)

    def learning(self, max_iter=100):

        self.compute_grad(True)
        params = [self.task_kernel_params] + [self.kernel.params] + [self.sigma]
        optimizer = torch.optim.Adam(params, lr=0.05)
        # optimizer = torch.optim.LBFGS(params, history_size=20, max_iter=20)
