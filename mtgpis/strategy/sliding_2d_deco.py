...
if __name__=="__main__":
    ...
    while True:
        ...
        if plot:
            ...
            set_plt()
            if save_movie:
                plt.savefig('../data/deco/mtgpis/movie/step{}.png'.format(movie_num))
                movie_num += 1
            # plt.show()
            plt.draw()
            plt.pause(0.001)
    ...
    for i in range(145):
        ...
        if save_data:
            set_plt()
            plt.scatter(surf_x, surf_y, s=5, color=(1.0, 0.0, 0.0))
            plt.savefig('../data/deco/mtgpis/estimate_surface/step{}.png'.format(i+1))
            plt.clf()

            # set_plt()
            # plt.plot(x2_0, y2_0, linewidth=3, color=(1.0, 0.0, 0.0))
            # plt.savefig('../data/deco/mtgpis/true_surface/step{}.png'.format(i+1))
            # plt.clf()
    ...
    if save_data:
        np.save('../data/deco/mtgpis/value/var', var_list)
        np.save('../data/deco/mtgpis/value/simirality', simirality_list)
