...
def judge_square(po):
    if (0.12 < po[0] < 0.48) and (0.12 < po[1] < 0.48):
        return -1
    else:
        return 1
...
if __name__=="__main__":
    ...
    movie_num = 1
    while True:
        if judge_square(current_po - 0.002) == -1:
            break
        current_po[1] -= 0.002
        ...
        if plot:
            ...
            set_plt()
            if save_movie:
                plt.savefig('../data/high_high/gpis/movie/step{}.png'.format(movie_num))
                movie_num += 1
            plt.draw()
            plt.pause(0.001)
            plt.clf()
    ...
    # X2 = np.append(X2, [current_po + np.array([0.3, 0])], axis=0)
    # Y2 = np.append(Y2, [1])[:,None]
    ...
    for i in range(140):
        ...
        if save_data:
            ...
            plt.savefig('../data/high_high/gpis/estimate_surface/step{}.png'.format(i+1))
            plt.clf()

            # set_plt()
            # plt.plot(x2_0, y2_0, linewidth=3, color=(1.0, 0.0, 0.0))
            # plt.savefig('../data/high_high/gpis/true_surface/step{}.png'.format(i+1))
            # plt.clf()
    ...
    if save_data:
        np.save('../data/high_high/gpis/value/var', var_list)
