...
def judge_square(po):
    if (0.32 < po[0] < 0.48) and (0.22 < po[1] < 0.48):
        return -1
    else:
        return 1
...
if __name__=="__main__":
    ...
    # true object
    N2 = 10
    x2_0 = (np.concatenate([np.linspace(32, 48, N2), np.ones(N2)*48, np.linspace(48, 32, N2), np.ones(N2)*32]) ) * rate
    ...
    while True:
        ...
        if plot:
            ...
            set_plt()
            if save_movie:
                plt.savefig('../data/low_high/mtgpis/movie/step{}.png'.format(movie_num))
                movie_num += 1
            plt.draw()
            plt.pause(0.001)
            plt.clf()
    ...
    for i in range(80):
        ...
        if i == 0:
            task_kernel_params = torch.tensor([[-0.2458, -1.516], [0.27, 0.0842]])
        else:
        ...
        if save_data:
            ...
            # plt.savefig('../data/low_high/mtgpis/estimate_surface/step{}.png'.format(i+1))
            # plt.clf()

            set_plt()
            plt.plot(x2_0, y2_0, linewidth=3, color=(1.0, 0.0, 0.0))
            plt.savefig('../data/low_high/mtgpis/true_surface/step{}.png'.format(i+1))
            plt.clf()
    ...
    if save_data:
        np.save('../data/low_high/mtgpis/value/var', var_list)
        np.save('../data/low_high/mtgpis/value/simirality', simirality_list)
