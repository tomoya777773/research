...
def judge_square(po):
    if (0.25 < po[0] < 0.35) and (0.25 < po[1] < 0.35):
        return -1
    else:
        return 1
...
if __name__=="__main__":
    ...
    while True:
        ...
        if plot:
            ...
            set_plt()
            if save_movie:
                plt.savefig('../data/low_low/mtgpis/movie/step{}.png'.format(movie_num))
                movie_num += 1
    ...
    for i in range(80):
        ...
        if save_data:
            ...
            plt.savefig('../data/low_low/mtgpis/estimate_surface/step{}.png'.format(i+1))
            plt.clf()

            # set_plt()
            # plt.plot(x2_0, y2_0, linewidth=3, color=(1.0, 0.0, 0.0))
            # plt.savefig('../data/low_low/mtgpis/true_surface/step{}.png'.format(i+1))
            # plt.clf()
    ...
    if save_data:
        np.save('../data/low_low/mtgpis/value/var', var_list)
        np.save('../data/low_low/mtgpis/value/simirality', simirality_list)
