...
class Path_Planning:
    ...
    def direction_func(self, current_position, orbit_position):
        ...
        diff_orbit = (orbit_position - current_position) / np.linalg.norm(orbit_position - current_position, ord=2)
...
class VREP_UR5:
    ...
    def __init__(self):
        ...
        # print self.X
        self.X[:, 2] = self.X[:, 2] - 0.0014712069858160426
        # print self.X
        self.Y = np.zeros((self.X.shape[0], 1))

        self.X = np.vstack((self.X, np.array([0.4, 0.01, 1.4])))
