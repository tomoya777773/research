...
class Path_Planning:
    ...
    def kernel_func(self, data_position, current_position):
        kernel_x = pow(np.linalg.norm(data_position - current_position)**2 + length**2 , -1/2)
        return kernel_x

    def diff_kernel_func(self, data_position, current_position):
        diff_kernel_x = - pow(np.linalg.norm(data_position - current_position)**2 + length**2, -3/2) * (data_position - current_position)
    ...
    def decision_func(self, po_list):
        ...
        cc = 0
        if la[0] < 0 and la[1] < 0: # 上に凸のとき
            cc = 1

        return cc

    """法線方向"""
    def direction_func(self, current_position, orbit_position):
        ...
        """軌道の微分"""
        diff_orbit = (current_position - orbit_position) / np.linalg.norm(current_position - orbit_position)
        print "diff_orbit: ", diff_orbit

        """ペナルティの微分"""
        if current_position[2] < z_limit:
            diff_penalty = np.array([[0], [0], [-2*c*(current_position[2] - z_limit)]])
        else:
...
class VREP_UR5:
    ...
    def __init__(self):
        ...
        self.position.data = np.array([4.25000000e-01, 0,1.4])

        """データを読み込み"""
        self.X = np.load("spheroid_list.npy")
        self.Y = np.zeros((self.X.shape[0], 1))

        self.orbit_position = np.load("circle_list.npy")
        self.force = 0
    ...
    def main(self):
        ...
        for i in tqdm(range(1)):
            ...
            current_position = self.arm_position
            ...
            z = np.linspace(current_position[2] - 0.002, current_position[2] + 0.002, w)
            ...
            if la == 1: # When convex
                ...
                pred_position = current_position + a * pred_direction
                ...
                if self.force == 0:
                    ...
                    n = 1
                    while n < 100:
                        normal_position = pred_position - np.array([0, 0, 0.01]) * n
                        # normal_position = self.X[self.X.shape[0]-1] + 0.01 * n * pred_normal.T[0]
                ...
                normal_position = current_position + np.array([0, 0, 0.01])
                # print current_position
                # print normal_position
                # normal_position = self.X[self.X.shape[0]-1] - 0.01 * pred_normal.T[0]
                self.position.data = normal_position
                self.publisher_position(self.position)
                # self.X = np.vstack((self.X, self.arm_position))
                # self.Y = np.vstack((self.Y, [-1]))
                ...
                n = 1
                while n < 10:
                    new_position = normal_position + n * 0.001 * pred_direction
                ...
                if self.force == 0:
                    n = 1
                    while n < 100:
                        normal_position = new_position - np.array([0, 0, 0.002]) * n
