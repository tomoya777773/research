...
file_path = "../data/high_low/sim_fixed"


def judge_square(po):
    if (0.218 < po[0] < 0.382) and (0.298 < po[1] < 0.482):
        return -1
    else:
        return 1
...
if __name__=="__main__":
    ...
    movie_num = 1
    while True:
        if judge_square(current_po - np.array([0.001, 0.002])) == -1:
            break
        current_po[1] -= 0.002
        ...
        if plot:
            ...
            inner = pat.Rectangle(xy = (0.22, 0.3), width = 0.16, height = 0.18, color=color_cycle[5])
            ax.add_patch(back)
            ax.add_patch(outer)
            ax.add_patch(inner)
